...
> **Note on R² thresholds**: The 0.15 value above is an *absolute plausibility ceiling* — the upper bound on what good financial models can achieve on returns. This differs from the "suspicious" calibration threshold in `src/config/domains.json` (`r2_returns` field), which flags claims that are implausibly high. Always consult `domains.json` for current thresholds — the values there are the source of truth. In practice: R² on returns in 0.01–0.15 is plausible, beyond that requires strong justification, and exceeding the `domains.json` suspicious bound triggers a red flag.
