...
| Warning demands zero cost or behavior change from its own audience, platform, or funders | Counterfeit-witness indicator: alarm produced where it is safe to produce | Cap the source's warning LR at 1.5 (per `references/evidence-calibration.md` LR conventions) until a self-costly claim is on record |

**Validation test:** Each row predicts a documentary observable at a deadline; for the first row, the absence of any incentive or personnel change at the institution's own stated deadline, verified against the record. For the safe-warning row, enumerate the warning's demanded actions and check whether any binds the issuer's own audience, platform, or funders — a checkable property of the warning text. A row's flag is DISCONFIRMED when the record shows the predicted absence did not hold: an incentive or personnel change landed by the deadline; the reform was evaluated by an independent unit; the category is still tracked and reported; response effort tracked severity rather than publicity; a process change is documented; the warning text contains a demand binding its own audience, platform, or funders.
