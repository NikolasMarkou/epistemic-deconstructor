...
**Forecasting vs. Sysid**: Use `forecast_modeler.py` when the deliverable is future values with calibrated intervals. Use `parametric_identifier.py` when the deliverable is **structure + parameters + uncertainty** for Phase 4 simulation or Phase 5 validation. The full fit JSON (`FitResult.to_json()`, which embeds the simulator-ready dict under its `simulator_format` key) is written to `$($SM path phase_3_model.json)` for downstream consumption by ed-model-synthesizer.
...
Write the full fit JSON (the `--output` file, i.e. `FitResult.to_json()`: parameter estimates + bootstrap CIs, AIC/BIC/AICc/FPE, Ljung-Box whiteness, walk-forward CV R², residual metadata) to `$($SM path phase_3_model.json)` so Phase 4 can locate it — it embeds the simulator-ready dict under the `simulator_format` key. Simplest: `$PID fit ... --output $($SM path phase_3_model.json)`.
