...
3. **Exit Gate Verification**: Before ANY phase transition, verify all required files exist and conditions are met. Phase 0 gate MUST include `[H_S]` and `[H_S_prime]` in `hypotheses.json`; additionally, the mechanical gate (`phase_gate.py` predicate `hypotheses_json_seeded`) requires **>=3 total entries** in `hypotheses.json` — the `[H_S]`/`[H_S_prime]` pair alone (2 entries) does NOT pass; at least one substantive system hypothesis must also be seeded. Phase 0.3 gate (when triggered) MUST include `domain_glossary.md`, `domain_metrics.json`, `domain_sources.md`, `phase_outputs/phase_0_3.md`, AND `domain_orienter.py gate` returns PASS (≥10 grounded terms / ≥3 metrics / ≥2 verified sources / library_fraction ≥0.30). Phase 0.7 gate: the machine-gated file is `scope_audit.json` (`candidates_unique >= 3` and `has_archetype_query`, checked by `scope_auditor.py gate`); `scope_audit.md` is the separate human-readable deliverable, not machine-checked. Phase 1.5 gate MUST include `phase_outputs/phase_1_5.md`, ≥3 observations inverted, surplus audit run, and ≥1 closed inference chain per promoted candidate (or an explicit "no promotion warranted" attestation).
4. **User Interaction**: Present findings, ask clarifying questions, get decisions.
   **Plain-Language Presentation.** At every phase handoff and the final summary, LEAD the user-visible message with a one-line plain-English **"Bottom Line:"** — what just happened, what it means, and the current best guess with how sure we are (in words AND as a %). Interpret every number in words; define each jargon term in plain words on first use. The technical content (hypothesis IDs, posteriors, tables, verdict tokens) is RETAINED VERBATIM directly beneath the Bottom Line (progressive disclosure).
   This is a **REGISTER on already-sanctioned surfaces ONLY** (per the Plain-Language Mandate in SKILL.md). It does NOT create a new report surface: rules (3) and (6) under "What You Do NOT Do" still hold — no output-format/depth question, and no monolithic reports / "inline findings" / synthesis outside Phase 5. Canonical convention in `references/plain-language-layer.md`.
...
8. Emit to user: the plain-language **Bottom Line** (one line — what this phase found, what it means, current lead + confidence in plain words) immediately followed by the protocol state block. Per `references/plain-language-layer.md`; the `[STATE: ...]` block stays verbatim beneath.
...
| PSYCH | Behavioral analysis | **orchestrator** dispatches: [**ed-domain-orienter** (P0-P.3, cultural-vocabulary scoping when triggered)] → [**ed-scope-auditor** (P0-P.7)] → [**ed-abductive-engine** (P1-P.5, behavioral_deviation category)] → **ed-psych-profiler** (owns P0-P through P5-P — framing, baseline, stimulus-response, structural ID, motive, validation — with 0-P.3 / 0-P.7 / 1-P.5 carved out to the sub-phase agents as shown). ed-psych-profiler does not spawn sub-agents; orchestrator sequences the pluggable phases itself when PSYCH tier is active. |
