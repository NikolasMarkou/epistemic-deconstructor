...
5. **TG**: for each candidate worth grounding, run `domain_orienter.py --file $($SM path domain_orientation.json) ground --term "<text>" --definition "<def>" --source <library|analyst|llm_parametric> [--url <url>]`. Prefer `library` sources; fall back to `analyst` only for terms inside your expertise; use `llm_parametric` last (capped at 0.60). Use WebFetch to consult external references when grounding library sources. **Fan-out ceiling**: ground at most 15 terms; stop early when `grounded_terms >= 10` AND `library_sourced_fraction >= 0.30`. Every 5 groundings, run `domain_orienter.py --file $($SM path domain_orientation.json) gate --json` and parse `grounded_terms` / `library_sourced_fraction`; early-exit the TG loop once both thresholds are met.
6. **MM**: `domain_orienter.py --file $($SM path domain_orientation.json) add-metric --name <n> --units <u> --higher-is-better <bool> --plausibility <sus,lo,hi,exc> --source <...> [--url <url>] --domain <domain>` for each canonical metric. LITE tier may skip. **Fan-out ceiling**: add at most 6 metrics; stop when `metrics_promoted >= 3`.
...
8. **CS**: `domain_orienter.py --file $($SM path domain_orientation.json) source --title <t> --category <...> [--url <url>] [--authors <a>] [--year <y>]` for each canonical reference. Then `domain_orienter.py --file $($SM path domain_orientation.json) verify --source-id <sid> --http-status <N>` with the WebFetch status you obtained. Minimum 2 verified sources. **Fan-out ceiling**: register at most 5 sources; stop when `verified_sources >= 2`. WebFetch calls for source HTTP-status checks may be batched in parallel; the subsequent `source` + `verify` Bash calls remain sequential per the Parallelism class block.
